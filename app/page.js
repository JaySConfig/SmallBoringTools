...
async function getLatestTools() {
  try {
    await connectMongo()
    const tools = await Tools.find().sort({ submittedAt: -1 }).lean()
    return tools
  } catch (error) {
    console.error('Error fetching tools:', error)
    return []
...
export default async function HomePage() {
  const tools = await getLatestTools()  // Rename from projects to tools
  ...
  return (
    <main className="max-w-4xl mx-auto p-8">
      ...
      <div className="mt-12">
        ...
        <div className="space-y-4">
          {tools.map(tool => (
            <div key={tool._id} className="p-4 border border-gray-200 rounded-sm">
              <div className="flex justify-between items-start">
                <h3 className="font-light">{tool.title}</h3>
                <span className="text-xs text-gray-500 font-light">
                  {tool.boringRating}
                </span>
              </div>
